  ...
  return (
    <Card className="grid gap-2 p-2">
      {products.map((product) => (
        <div
          key={product._id}
          className="flex flex-col sm:flex-row justify-between items-start sm:items-center border p-3 rounded-lg bg-gray-100 dark:bg-zinc-900"
        >
          <div className="flex items-start gap-3 w-full sm:w-auto">
            ...
            <div className="grid gap-1 text-sm sm:text-base">
              <h1 className="font-semibold">{product?.id?.name}</h1>
              <div className="flex flex-wrap items-center gap-2 text-xs text-gray-500">
                ...
                <span>
                  Status:
                  <span className="font-medium text-customYellow ml-1">
                    {status}
                  </span>
                </span>
              </div>
          ...
          <div className="mt-3 sm:mt-0 flex sm:flex-col items-end gap-2 text-right">
            <h2 className="text-md sm:text-xl font-bold flex items-center text-customYellow">
              <IndianRupee size={18} />
              {product?.id?.price}
            </h2>
            <p className="dark:text-customYellow text-sm">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center text-sm sm:text-base">
        <span>
          Order On: <span>{new Date(createdAt).toLocaleString()}</span>
        </span>
        <span
          onClick={handleDownloadInvoice}
          className="hover:underline cursor-pointer flex items-center gap-1 dark:text-customYellow mt-2 sm:mt-0"
        >
